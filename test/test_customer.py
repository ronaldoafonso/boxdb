...
class TestCustomerGet:
    ...
    def test_get_non_existing_customer(self, remove_all_customers):
        r = requests.get('http://localhost:5000/v1/customers/customer%201')
        rbody = r.json()
...
class TestCustomerPut:
    ...
    def test_put_an_non_existing_customer(self, remove_all_customers):
        r = requests.put('http://localhost:5000/v1/customers/customer%201', json={'boxes': ['box1']})
        rbody = r.json()
        assert r.status_code == 404
        assert r.headers['content-type'] == 'application/json'
    ...
    def test_put_a_customer_with_one_box(self, add_customers):
        add_customers.add([{'name': 'customer 1', 'boxes': ['box1']}])
        r = requests.put('http://localhost:5000/v1/customers/customer%201', json={'boxes': ['box1', 'box2']})
        rbody = r.json()
        rdata = json.loads(subprocess.check_output('docker container exec boxdb_boxdb-mongo_1 mongo --quiet --eval \'var customer="customer 1"\' /root/data/check_customer.js', shell=True))
        assert r.status_code == 200
        assert r.headers['content-type'] == 'application/json'
...
class TestCustomerDelete:
    ...
    def test_delete_an_non_existing_customer(self, remove_all_customers):
        r = requests.delete('http://localhost:5000/v1/customers/customer%201')
        rbody = r.json()
        rdata = json.loads(subprocess.check_output('docker container exec boxdb_boxdb-mongo_1 mongo --quiet --eval \'var customer="customer 1"\' /root/data/check_customer.js', shell=True))
        assert r.status_code == 200
        assert r.headers['content-type'] == 'application/json'
